...
public class UnpaidFragment extends BaseFragment implements IOrderslListener, ServiceListener, XRecyclerView.LoadingListener, View.OnClickListener {

    /**
     * 下拉刷新列表
     */
    private XRecyclerView mUnpaidPtrlvOrders = null;
    /**
     * 当前未付款订单
     */
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    // 设置停止刷新
                    mUnpaidPtrlvOrders.refreshComplete();
                    break;
            }
        }
    };
    ...
    protected void initView() {
        ...
        if (isInit) {
            ...
            if (isNeedRefresh) {
                // 需要刷新
                viewOrderList(info.getNotpay().getOrderList(), page);
//                mUnpaidPtrlvOrders.getRefreshableView().setEmptyView(mEmptyView);
                // 设置刷新标记为已刷新
                isNeedRefresh = false;
            }
        }
    ...
    private void viewOrderList(ArrayList<OrderDetail> list,int _page) {
        if (total < 1) {
            ...
            if (go_shopping != null || mUnpaidPtrlvOrders == null) {
                setContentView(R.layout.fragment_all);
                mUnpaidPtrlvOrders = (XRecyclerView) mMainView.findViewById(R.id.all_ptrlv_orders);
                LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
                linearLayoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                mUnpaidPtrlvOrders.setLayoutManager(linearLayoutManager);
                // 设置刷新模式监听
                mUnpaidPtrlvOrders.setLoadingListener(this);
            }
            if (null == list) {
                if (_page > 1) {
                    page--;
                ...
                // 停止刷新
                mUnpaidPtrlvOrders.loadMoreComplete();
                return;
            }
            // 如果是刷新模式或者是第一次加载
            if (_page == 1) {
                ...
                // 设置适配器
                mUnpaidPtrlvOrders.setAdapter(adapter);
            ...
            if (list.size() < num) {
                // 只允许上拉刷新
                mUnpaidPtrlvOrders.setLoadingMoreEnabled(false);

                mUnpaidPtrlvOrders.noMoreLoading();
                mUnpaidPtrlvOrders.removeFootView();
            } else {
                // 允许双向刷新加载
                mUnpaidPtrlvOrders.setLoadingMoreEnabled(true);
            }
            go_shopping = null;
        }
        ...
        switch (action) {
            case ORDER_LIST:
                ...
                if (getActivity() != null && !getActivity().isFinishing()) {
                    // 展示列表
                    viewOrderList(info.getNotpay().getOrderList(),p);
                } else {
                    // 设置游离模式下的刷新监控器
                    isNeedRefresh = true;
                }
...
//                    if (isNeedRefreshing && !mUnpaidPtrlvOrders.isRefreshing()) {
//                        mLoadingDialog.show();
//                    } else {
//                        mLoadingDialog.dismiss();
//                    }
