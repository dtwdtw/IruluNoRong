...
public class WishAdapter extends BaseAdapter{
    ...
    /** wish列表*/
    private ArrayList<WishInfo> mWishList;
    private LayoutInflater mInflater;
    private Picasso mPicasso;

    public WishAdapter(Context context, ArrayList<WishInfo> wishList) {
        mContext = context;
        mWishList = wishList;
    ...
    public void refresh(ArrayList<WishInfo> pWishList){
...
//        return 0;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null){
            ...
            mHolder.mWishTvRemoved = (TextView) convertView.findViewById(R.id.wish_tv_removed);
            convertView.setTag(mHolder);
        }else{
            mHolder = (ViewHolder) convertView.getTag();
        }
        mHolder.mWishTvName.setText(info.getName());
        mHolder.mWishTvPrice.setText(StringUtils.getPriceByFormat(info.getCurrPrice()));
        mHolder.mWishTvTotal.setText(String.valueOf(info.getCommentNum()));
        mHolder.mWishRbRate.setRating(info.getStar());
        if (info.getStatus() == 1){
            mHolder.mWishTvRemoved.setVisibility(View.GONE);
        }else{
            mHolder.mWishTvRemoved.setVisibility(View.VISIBLE);
        }
