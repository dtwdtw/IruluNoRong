package com.wf.irulu.module.homenew1_3_0.HotSalesFragment;
 ...
 */
public class HotSalesFragment1_3_0 extends BaseContentFragment implements ServiceListener, IsAddWishListListener, XRecyclerView.LoadingListener, View.OnClickListener {
    private final String TAG = getClass().getCanonicalName();
    private PullToRefreshHeaderGridView mPullToRefreshListView;
    private String mId;
    private HotsalesAdapter hotsalesAdapter;
    private HomeBean.HotSalesBean hotSalesBean;
...
//        mRecyclerView.setArrowImageView(R.mipmap.default_ptr_flip);
    ...
    public void loadData(HomeBean homeBean) {
        hotSalesBean = homeBean.getHot_sales();
        mProductList=hotSalesBean.getProductList();
    }

    private void initData() {
        if (hotSalesBean == null) {
            return;
        }
        boolean tag = true;//让mPullToRefreshListView只添加一次头布局
        String type = hotSalesBean.getType();
        if ("2".equals(type)) {
            mId = hotSalesBean.getId();
            mProductList = hotSalesBean.getProductList();
        ...
        hotsalesAdapter = new HotsalesAdapter(mActivity, mProductList, controller);
        mRecyclerView.setAdapter(hotsalesAdapter);
    }
        ...
        switch (action) {
            case HOME_MORE:
                ...
                if (page == 1) {//下拉刷新
                    ...
                    if (null != mCyclicView) {
                        setShufflingFigure(mActivity, mCyclicView, slideDatas);
                    }
                    hotsalesAdapter.setNotifyDataSetChanged(mProductList);
