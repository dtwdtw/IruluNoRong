...
public class ShippedFragment extends BaseFragment implements IOrderslListener, ServiceListener, XRecyclerView.LoadingListener, View.OnClickListener {

    /**
     * 下拉刷新列表
     */
    private XRecyclerView mShippedPtrlvOrders = null;
    /**
     * 当前已发货订单
     */
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    // 设置停止刷新
                    mShippedPtrlvOrders.refreshComplete();
                    break;
            }
        }
    };
    ...
    protected void initView() {
        ...
        if (isInit) {
            ...
            if (isNeedRefresh) {
                // 需要刷新
                viewOrderList(info.getNotconfirm().getOrderList(),page);
//                mShippedPtrlvOrders.setEmptyView(mEmptyView);
                // 设置刷新标记为已刷新
                isNeedRefresh = false;
            }
        }
    ...
    private void viewOrderList(ArrayList<OrderDetail> list,int _page) {
        if (total < 1) {
            ...
            if (go_shopping != null || mShippedPtrlvOrders == null) {
                setContentView(R.layout.fragment_all);
                mShippedPtrlvOrders = (XRecyclerView) mMainView.findViewById(R.id.all_ptrlv_orders);
                LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
                linearLayoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                mShippedPtrlvOrders.setLayoutManager(linearLayoutManager);
                // 设置刷新模式监听
                mShippedPtrlvOrders.setLoadingListener(this);
            }
            if (null == list) {
                if (_page > 1) {
                    page--;
                ...
                // 停止刷新
                mShippedPtrlvOrders.loadMoreComplete();
                return;
            }
            // 如果是刷新模式或者是第一次加载
            if (_page == 1) {
                ...
                // 设置适配器
                mShippedPtrlvOrders.setAdapter(adapter);
            ...
            if (list.size() < num) {
                // 只允许上拉刷新
                mShippedPtrlvOrders.setLoadingMoreEnabled(false);

                mShippedPtrlvOrders.noMoreLoading();
                mShippedPtrlvOrders.removeFootView();
            } else {
                // 允许双向刷新加载
                mShippedPtrlvOrders.setLoadingMoreEnabled(true);
            }
            go_shopping = null;
        }
        ...
        switch (action) {
            case ORDER_LIST:
                ...
                if (getActivity() != null && !getActivity().isFinishing()) {
                    // 展示列表
                    viewOrderList(info.getNotconfirm().getOrderList(),p);
                } else {
...
//                    if (isNeedRefreshing && !mShippedPtrlvOrders.isRefreshing()) {
//                        mLoadingDialog.show();
//                    } else {
//                        mLoadingDialog.dismiss();
//                    }
