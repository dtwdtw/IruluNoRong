...
public abstract class BaseSliderView {
    ...
    protected BaseSliderView(Context context,ImageView imageView) {
        mContext = context;
    ...
    /**
     * set saving and loading pattern of image, in and from cache or not.
     * @param isNeedCache is need to be saved and search from cache
     * @return
     */
    public BaseSliderView setCache(boolean isNeedCache){
    ...
    protected void bindEventAndShow(final View v, ImageView targetImageView){
        ...
        mImageView = targetImageView;
        mProgressBar = (ProgressBar) v.findViewById(R.id.loading_bar);
        showDataView(mImageView);
   }

    public void showDataView(ImageView imageView){
        if (mImageView == null && imageView == null){
            return;
        }
        mPicasso = (mPicasso != null) ? mPicasso : Picasso.with(mContext);
        if (mImageView == null){
            mImageView = imageView;
        }
        ...
        if(mUrl!=null){
            rq = mPicasso.load(mUrl);
        }else if(mFile != null){
            rq = mPicasso.load(mFile);
        }else if(mRes != 0){
            rq = mPicasso.load(mRes);
        }else{
            // return;
            // 此处修改源码,以保证当url为空时执行加载占位图片
            rq = mPicasso.load(R.mipmap.notify_image_xiaotu);
        }
        ...
        rq.into(mImageView,new Callback() {
            @Override
            ...
            public void onError() {
                if(mLoadListener != null){
                    mLoadListener.onEnd(false,BaseSliderView.this);
                }
                if(mProgressBar != null){
                    mProgressBar.setVisibility(View.INVISIBLE);
                }
