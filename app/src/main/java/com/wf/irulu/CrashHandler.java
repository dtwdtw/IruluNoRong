...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	/** 错误报告文件的后缀扩展名 */  
    private static final String CRASH_REPORTER_EXTENSION = "-irulu.cr";
...
//		Thread.setDefaultUncaughtExceptionHandler(this);//用这个方法，现在这个APP捕获不到异常，
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		ILog.e(TAG, " UncaughtExceptionHandler!");
...
//			}
			exit();
			MobclickAgent.onKillProcess(mContext);//如果开发者调用 Process.kill 或者 System.exit 之类的方法杀死进程，请务必在此之前调用此方法，用来保存统计数据。
	...
	private boolean handleException(Throwable ex) {
		...
		ILog.i(TAG, " handleException in CrashHandler !!");
...
//        }.start();
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				...
				if (FileUtils.hasSDCard()) {
					filePath = CacheManager.SD_CRASH_DIR + File.separator + fileName;
				}
				ILog.e(TAG, "文件路径="+filePath);
			...
			ILog.e(TAG, "an error occured while writing file...", e);  
		}
	...
	public void collectDeviceInfo(Context context) {
		try {
			...
			ILog.e(TAG, "an error occured when collect package mInfo", e);
		}
		...
		for (Field field : fields) {
			try {
				...
				ILog.e(TAG, field.getName() + ":" + field.get(""));
			 } catch (Exception e) {  
	            ILog.e(TAG, "an error occured when collect crash mInfo", e);
	         }  
		}
	...
	private void exit() {
		ILog.d(TAG, "Exit On Global Exception");
        String packageName = IruluApplication.getInstance().getPackageName();
        String processId = "";
        try {
        	...
        	e.printStackTrace();
        	ILog.e(TAG, "uncaughtException: ", e);
        }
    }
